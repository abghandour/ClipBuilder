#!/usr/bin/env python3
"""Compare finishing ranges on the edit path with the single full pass in one Release binary.
    ...
    for line in log.splitlines():
        if line.startswith("PHASE START "):
            ...
            phases[current] = dict(segmentHits=0, finishingHits=0, fullPasses=0, rangeHits=0,
                                   rangeEncodes=0, rangeFailures=0, fallbacks=0,
...
def main():
    ...
    for pair in range(1, args.pairs + 1):
        for mode in (["control", "ranges"] if pair % 2 else ["ranges", "control"]):
            ...
            command = [sys.executable, str(script), "record", "--scenario", "render",
                       ...
                       "--incremental-finishing", "off" if mode == "control" else "editsOnly"]
            ...
            if mode == "control":
                assert not any(p["rangeEncodes"] or p["rangeHits"] for p in phases)
                assert byName["render-caption-edit"]["fullPasses"] == 1
            else:
                assert byName["render-cold"]["fullPasses"] == 1 and byName["render-cold"]["rangeEncodes"] == 0
                assert byName["render-caption-edit"]["rangeHits"] == 0 and byName["render-caption-edit"]["rangeEncodes"] >= 2
                assert byName["render-second-edit"]["rangeEncodes"] >= 1 and byName["render-second-edit"]["rangeHits"] >= 1
                assert byName["render-second-edit"]["fullPasses"] == 0
            hashes = {}
            for name in PHASES:
    ...
    for mode in ["control", "ranges"]:
        ...
        for phase in PHASES:
            ...
            summary[mode]["phases"][phase] = dict(
                ...
                fullPasses=[p["fullPasses"] for p in phases])
